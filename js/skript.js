...
function loadToDo() {
  ...
  const getThemeByTime = () => {
    ...
    return hourse >= 7 && hourse < 19 ? "light" : "dark";
  };
  ...
  if (saveTheme) {
    ...
    const setingsThemeOp = themeSetingsOp();
    if (setingsThemeOp === "dark") {
      setTheme("dark");
    } else {
      const thisTyme = getThemeByTime();
      setTheme(thisTyme);
    }
  }
